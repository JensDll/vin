...
class EventListener final : public peel::Gio::Initable
{
  PEEL_SIMPLE_CLASS(EventListener, Initable)

  friend class peel::Gio::Initable;

  using SignalEvent = peel::Signal<EventListener, void(const char*)>;
...
public:
  ...
  {
    const auto result{ peel::Object::create<EventListener>(
      prop_main_context(), main_context, prop_worker_context(), worker_context) };
    result->init(cancellable, error);
    return result;
  }

  static auto create()
  {
    return peel::Object::create<EventListener>();
  }
