...
#include "vin/hyprland/error.hpp"
...
template<Connect C>
int connect(peel::UniquePtr<peel::GLib::Error>* const error)
{
  const char* const runtime_dir{ std::getenv("XDG_RUNTIME_DIR") };

  if (runtime_dir == nullptr) {
    peel::GLib::set_error_literal(
      error, s_quark, static_cast<int>(Error::env_no_xdg_runtime_dir), "Failed to determine XDG_RUNTIME_DIR");
    return -1;
  }
  ...
  if (his == nullptr) {
    peel::GLib::set_error_literal(error,
      s_quark,
      static_cast<int>(Error::env_no_hyprland_instance_signature),
  ...
  const auto result{ fmt::format_to_n(
    static_cast<char*>(addr.sun_path), sizeof(addr.sun_path), connect_path<C>(), runtime_dir, his) };
