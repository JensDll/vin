...
public:
  ...
  [[nodiscard]] std::string_view app_icon() const
  {
    return m_app_icon;
  }
  ...
  [[nodiscard]] guint32 replaces_id() const
  {
    return m_replaces_id;
  }

  [[nodiscard]] gint32 expire_timeout() const
  {
    return m_expire_timeout;
  }

  [[nodiscard]] Urgency urgency_hint() const
  {
    return m_urgency_hint;
  }

  [[nodiscard]] std::string_view category_hint() const
  {
    return m_category_hint;
  }

  [[nodiscard]] std::string_view desktop_entry_hint() const
  {
    return m_desktop_entry_hint;
  }

  [[nodiscard]] std::string_view action(const std::string_view key) const
  {
    const auto it{ m_actions.find(key) };
    return it == m_actions.end() ? "" : it->second;
  }

  [[nodiscard]] guint32& id()
  {
    return m_id;
  }

  [[nodiscard]] guint32 id() const
  {
    return m_id;
  }
};
